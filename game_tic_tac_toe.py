print('Правила: Игорк_1 ходит крестиками, Игрок_2 - ноликами. '
...
data_index_1 = set()      # Записываются координаты координах ходов Игрока_1.
data_index_2 = set()      # Записываются координаты координах ходов Игрока_2.
...
def play():
    global x, y
    if k % 2:
        print('Игорк_2 делает ход: столбец № ', end='')
    else:
        print('Игорк_1 делает ход: столбец № ', end='')
    x = int(input())
    while x not in (0, 1, 2):
        print('Вы ввели не правильную координату, повторите ввод: ', end='')
        x = int(input())
    print('                     строка № ', end='')
    y = int(input())
...
# Пока не соберется выиграшная комбинация:
winner = False
while not winner:
    if k == 9:
